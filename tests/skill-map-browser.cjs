...
;(async () => {
  ...
  try {
    ...
    await page.getByRole('button', { name: /Pantalla completa|Full screen/ }).click()
    const expandedMap = page.locator('.skill-map.is-expanded')
    await expandedMap.waitFor()
    assert.equal(await expandedMap.getAttribute('role'), 'dialog')
    assert.equal(await page.evaluate(() => document.body.style.overflow), 'hidden')
    const expandedBox = await expandedMap.boundingBox()
    assert.ok(expandedBox.width >= 1439 && expandedBox.height >= 999, 'expanded map must fill the viewport')
    assert.equal(await expandedMap.locator('.skill-node[aria-pressed="true"]').count(), 1, 'selection must survive expansion')
    await page.keyboard.press('Escape')
    await page.locator('.skill-map.is-expanded').waitFor({ state: 'detached' })
    assert.equal(await page.evaluate(() => document.body.style.overflow), '')
    assert.equal(await page.locator('.skill-node[aria-pressed="true"]').count(), 1, 'selection must survive closing')
